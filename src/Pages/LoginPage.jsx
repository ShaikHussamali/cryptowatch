...
const LoginPage = () => {
  ...
  const handleLogin = (e) => {
    e.preventDefault();
    const credentialsvalue = JSON.parse(localStorage.getItem('credentials'));
    console.log(credentialsvalue)
    // get the user data from the localStorage if matches take the user to Home page
    if(credentials.email === credentialsvalue.email&&credentials.password === credentialsvalue.password){
      history.push('/home')
    }
    else  
